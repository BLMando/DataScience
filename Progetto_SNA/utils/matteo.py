...
emisfero_sinistro = cervello.drop(
    cervello[(cervello.side != "left") | (cervello.side.isnull())].index
)

emisfero_destro = cervello.drop(
    cervello[(cervello.side != "right") | (cervello.side.isnull())].index
)
...
condizione = (
    condizione + " & (emisfero_sinistro.hemibrain_type == nodo['hemibrain_type'])"
)
...
for index, nodo in emisfero_destro.iterrows():
    ...
    if i.size > 0:
        emisfero_destro = emisfero_destro.drop(
            emisfero_destro[(emisfero_destro.root_id == nodo["root_id"])].index
        )
...
def calcoloValoriMedi(G, insieme1, insieme2):
    ...
    new_row = {
        ...
        "vm_clustering": 0,
        "vm_in_degree_perc": 0,
        "vm_out_degree_perc": 0,
        "vm_clustering_perc": 0,
    }
    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
...
# %%
def disegnaGraficiAccoppiati(
    insieme1: pd.DataFrame, valori1, titolo1, insieme2, valori2, titolo2
):
    ...
    for opzione in opzioni:
        ...
        for i in range(2):
            ...
            numero_bin = (
                int(
                    (max(list(grafico[opzione])) - min(list(grafico[opzione])))
                    / bin_width
                )
                * 3
            )
        ...
        plt.savefig(
            "adulta/images/" + titolo_super + " " + titolo1 + " - " + titolo2 + ".png",
            format="png",
            bbox_inches="tight",
        )
        plt.show()
...
nodi_con_corrispettivo = cervello.drop(
    cervello[((cervello.side != "left") & (cervello.side != "right"))].index
)
for index, nodo in nodi_senza_corrispettivo.iterrows():
    nodi_con_corrispettivo = nodi_con_corrispettivo.drop(
        nodi_con_corrispettivo[
            (nodi_con_corrispettivo.root_id == nodo["root_id"])
        ].index
    )
print(len(nodi_con_corrispettivo))
contaOccorrenze(
    nodi_senza_corrispettivo,
    "Grafico a barre della frequenza per area dei nodi senza corrispettivo",
)
contaOccorrenze(
    nodi_con_corrispettivo,
    "Grafico a barre della frequenza per area dei nodi con corrispettivo",
)
contaOccorrenze(
    emisfero_sinistro,
    "Grafico a barre della frequenza per area dei nodi senza corrispettivo dell'emisfero sinistro",
)
contaOccorrenze(
    emisfero_destro,
    "Grafico a barre della frequenza per area dei nodi senza corrispettivo dell'emisfero destro",
)
...
nodi_senza_corrispettivo = calcoloPercentuali(
    G, nodi_senza_corrispettivo, nodi_con_corrispettivo
)
...
nodi_con_corrispettivo = calcoloPercentuali(
    G, nodi_con_corrispettivo, nodi_senza_corrispettivo
)
...
valori_medi_isolati = calcoloValoriMedi(
    G, nodi_senza_corrispettivo, nodi_con_corrispettivo
)
...
valori_medi_non_isolati = calcoloValoriMedi(
    G, nodi_con_corrispettivo, nodi_senza_corrispettivo
)
...
# %%
disegnaGraficiAccoppiati(
    nodi_senza_corrispettivo,
    valori_medi_isolati,
    "Nodi senza corrispettivo",
    nodi_con_corrispettivo,
    valori_medi_non_isolati,
    "Nodi con corrispettivo",
)
...
def ottieniMotifPerNodo(insieme):
    ...
    for index, nodo_in_esame in insieme.iterrows():
        ...
        for opzione in opzioni:
            nodi_vicini = eval(
                "np.array(list(G." + opzione + "_edges(nodo_in_esame['root_id'])))"
            )
            exec(
                "df_" + opzione + " = pd.DataFrame({'root_id': [], 'super_class': []})"
            )
            for nodo in nodi_vicini:
                nodo_area = cervello.loc[
                    cervello["root_id"] == nodo[posizione[opzione]]
                ]["super_class"]
                new_row = {
                    "root_id": nodo[posizione[opzione]],
                    "super_class": str(nodo_area.item()),
                }
                exec(
                    "df_"
                    + opzione
...
def contaOccorrenzeMotif(insieme):
    ...
    for motif in motifs:
        ...
        df_times_motif = pd.concat(
            [df_times_motif, pd.DataFrame([new_row])], ignore_index=True
        )

    df_times_motif = df_times_motif.sort_values(
        by=["times", "motif"], ascending=[False, True]
    )
...
df_times_motif_nodi_senza_corrispettivo = contaOccorrenzeMotif(
    df_motif_nodi_senza_corrispettivo
)

df_motif_nodi_con_corrispettivo = ottieniMotifPerNodo(nodi_con_corrispettivo)
df_times_motif_nodi_con_corrispettivo = contaOccorrenzeMotif(
    df_motif_nodi_con_corrispettivo
)

df_motif_nodi_senza_corrispettivo.to_csv(
    "adulta/csv/Elenco dei motif dei nodi senza corrispettivo.csv", index=False
)
df_times_motif_nodi_senza_corrispettivo.to_csv(
    "adulta/csv/Numero occorrenze dei motif dei nodi senza corrispettivo.csv",
    index=False,
)

df_motif_nodi_con_corrispettivo.to_csv(
    "adulta/csv/Elenco dei motif dei nodi con corrispettivo.csv", index=False
)
df_times_motif_nodi_con_corrispettivo.to_csv(
    "adulta/csv/Numero occorrenze dei motif dei nodi con corrispettivo.csv", index=False
)
...
graficoMotif(
    ...
    "Grafico a barre della frequenza per motif dei nodi senza corrispettivo",
    12,
)
graficoMotif(
    pd.read_csv(
        "adulta/csv/Numero occorrenze dei motif dei nodi con corrispettivo.csv"
    ),
    "Grafico a barre della frequenza per motif dei nodi con corrispettivo",
    220,
)
...
def disegnaSankey(insieme, titolo):
    ...
    for index, nodo in sankey.iterrows():
        valori_duplicati = sankey.drop(
            sankey[
                (sankey.source != nodo["source"]) & (sankey.target != nodo["target"])
            ].index
        )
        new_row = {
            "source": nodo["source"],
            "target": nodo["target"],
            "value": valori_duplicati["value"].sum(),
        }
...
# %%
df_times_motif_nodi_senza_corrispettivo = pd.read_csv(
    "adulta/csv/Numero occorrenze dei motif dei nodi senza corrispettivo.csv"
)
df_times_motif_nodi_con_corrispettivo = pd.read_csv(
    "adulta/csv/Numero occorrenze dei motif dei nodi con corrispettivo.csv"
)

# %%
disegnaSankey(
    df_times_motif_nodi_senza_corrispettivo,
    "Diagramma sankey dei motif dei nodi senza corrispettivo",
)

# %%
disegnaSankey(
    df_times_motif_nodi_con_corrispettivo,
    "Diagramma sankey dei motif dei nodi con corrispettivo",
)
